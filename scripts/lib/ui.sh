...
draw_module_logo() {
    ...
    # Внутренняя рамка (верх)
    echo "   ║  ╔═══════════════════════════════════════════════════╗  ║"

    # Вычисляем длину внутренней части (между двойными рамками)
    local sample="   ║  ╔═══════════════════════════════════════════════════╗  ║"
    # Убираем левую часть "   ║  ╔" и правую "╗  ║"
    local inner_part=$(echo "$sample" | sed -E 's/^ {3}║  ╔(.*)╗  ║$/\1/')
    local inner_width=${#inner_part}
    ...
    # Нижняя граница внутренней рамки (ровно inner_width символов ═)
    printf "   ║  ╚"
