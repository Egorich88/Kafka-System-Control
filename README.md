...
**Версия 3.1** | Разработано [Egorich88](https://github.com/Egorich88) (2023–2026)
...
- Топики – поиск, конфигурация, список, поиск сообщения.

- Группы потребителей – поиск, список, состояние всех групп, управление оффсетами.
...
🔄 Управление оффсетами (reset)
В меню «Группы потребителей» → «Управление оффсетами» можно выполнить сброс позиции группы:

1. Выбрать группу.
2. Выбрать один или все топики группы.
3. Выбрать тип сброса:

    - На самое начало (--to-earliest)

    - В самый конец (--to-latest)

    - Сдвиг на N сообщений (--shift-by N)

    - На конкретную дату/время (--to-datetime)

    - На конкретный оффсет (--to-offset с указанием партиции)

4. Сначала выполняется пробный запуск (--dry-run) для просмотра изменений, затем после подтверждения – фактический сброс (--execute).

Важно: Группа должна быть неактивной (все потребители остановлены), иначе сброс не удастся.
...
*   [x] Управление оффсетами групп (сброс на начало/конец/по времени)
